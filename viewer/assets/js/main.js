...
var main = (function($) { var _ = {
  ...
  /**
    ...
    // Return.
			...
			// Prevent event from bubbling up to "hide event on tap" event.
				_.$return.on('touchend', function(event) {
					event.stopPropagation();
				});
...
/*
    ...
    const delay = ms => new Promise(res => res(ms));

    const thumbnails = document.querySelector('#thumbnails');
    const response = await fetch('/data.json');
    const data = await response.json();

    const { images, text, caption } = 
      data.portfolios.find(e => 
        e.text.toLowerCase().replace(/ /g, '_') === location.hash.replace(/#/g, '')
      );

    document.querySelector('#title').innerHTML = text;
    document.title = 'Room 236 | ' + text;
    document.querySelector('#caption').innerHTML = caption.replace(/\*(.+?)\*/g, '<em>$1</em>').replace(/\n/g, '<br>');
    ...
    for (const url of images) {
      ...
      img.setAttribute('src', `images/thumbs/${url}`);
      
      anchor.classList.add('thumbnail');
      anchor.setAttribute('data-position', 'center center');
      anchor.setAttribute('href', `images/fulls/${url}`);
